var remjs = function() {
    ...
    var fn = function(width) {
        if (width <= 750) {
            ...
            // pc版本
            document.documentElement.style.fontSize = 100 * (width / 1920) + "px";
        }
    ...
    if (!checkLetIE9()) {
        // 1.获取 client 屏幕宽度
        var clien_width = document.documentElement.clientWidth || document.body.clientWidth;

        // 2.如果屏幕的宽度发生改变 触发 onresize 方法
        window.onresize = function() {
            clien_width = document.documentElement.clientWidth || document.body.clientWidth;
            fn(clien_width);
        }
